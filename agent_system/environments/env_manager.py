...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs):
        ...
        if self.retrieval_memory is not None and self._traj_use_with_memory:
            self.retrieved_memories = []
        ...
        observations = {'text': full_text_obs, 'text_base': full_text_obs, 'image': image_obs, 'anchor': text_obs, 'game_file': list(self.gamefile)}
        return observations, infos
    ...
    def step(self, text_actions: List[str]):
        ...
        next_observations = {'text': full_text_obs, 'text_base': full_text_obs, 'image': image_obs, 'anchor': text_obs, 'game_file': list(self.gamefile)}
        rewards = to_numpy(rewards)
        dones = to_numpy(dones)
...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        ...
        observations = {'text': full_text_obs,
                        ...
                        'image': None,
                        'anchor': obs.copy(),
                        # Traj-unlearn: format_obs() above strips the
                        # instruction out of 'anchor' (WebShop's GiGPO step
                        # grouping wants task-agnostic state matching), which
                        # leaves 'anchor' useless as a per-task identity for
                        # unlearn/unlearn_inject.py's forget/retain injection
                        # matching (see ForgetTrajectoryInjectingCollector's
                        # match_key docstring). 'task_key' carries the raw
                        # instruction text (self.tasks, captured before
                        # stripping) instead, for that purpose only.
                        'task_key': list(self.tasks),
                        }
        self.pre_text_obs = obs
        self.memory.reset(batch_size=len(infos))
        return observations, infos
    ...
    def build_text_obs(self, text_obs: List[str], infos: List[List[str]], init: bool = False) -> List[str]:
        ...
        use_retrieval = (
            ...
            and self._traj_use_with_memory
        )
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        # Optional override (e.g. to restrict env.alfworld.task_types for a
        # trajectory-unlearning trial on a single task type) via
        # env.alfworld.config_path=<path to an alternate config_tw*.yaml>.
        # Falls back to the original hardcoded path when unset, so existing
        # launch commands are unaffected.
        config_path_override = OmegaConf.select(config, "env.alfworld.config_path")
        if config_path_override:
            alf_config_path = config_path_override
        elif config.env.env_name == 'alfworld/AlfredThorEnv':
        ...
        env_kwargs = {
            ...
            # Traj-unlearn guaranteed-coverage forget pool (see AlfworldEnvs
            # in envs.py). Both no-op (None/0) unless explicitly set.
            'forget_alf_config_path': OmegaConf.select(config, "env.alfworld.forget_config_path"),
            'forget_env_num': OmegaConf.select(config, "env.alfworld.forget_env_num") or 0,
            'forget_game_files_path': OmegaConf.select(config, "env.alfworld.forget_game_files_path"),
            'exclude_game_files_path': OmegaConf.select(config, "env.alfworld.exclude_game_files_path"),
        }
        ...
        env_kwargs = {
                    ...
                    'file_path': file_path,
                    'attr_path': attr_path,
                    # Traj-unlearn: both no-op (None) unless explicitly set.
                    'include_task_keys_path': OmegaConf.select(config, "env.webshop.include_task_keys_path"),
                    'exclude_task_keys_path': OmegaConf.select(config, "env.webshop.exclude_task_keys_path"),
                    # Traj-unlearn (GiRPO guaranteed-coverage): both no-op
                    # (None/0) unless explicitly set -- see
                    # WebshopMultiProcessEnv's guaranteed-coverage cohort
                    # logic and ppo_trainer.yaml's env.webshop docstrings.
                    'guaranteed_forget_task_keys_path': OmegaConf.select(config, "env.webshop.guaranteed_forget_task_keys_path"),
                    'guaranteed_forget_slots': OmegaConf.select(config, "env.webshop.guaranteed_forget_slots") or 0,
                    }
