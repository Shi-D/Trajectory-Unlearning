...
def compute_teacher_token_advantage(
    ...
    """
    Compute token-level teacher advantage for Traj.
...
def summarize_traj_advantage_components(
    response_mask: torch.Tensor,
    ...
    """
    Summarize the relative contribution of the three Traj advantage terms.
    ...
    metrics = {}
    metrics.update(_summarize("traj/adv_raw", raw_components))
    metrics.update(_summarize("traj/adv", weighted_components))
    metrics.update({
        "traj/adv/teacher_active_token_ratio": teacher_active_token_ratio,
        "traj/adv/step_weight": float(step_advantage_w),
    })
    return metrics


def compute_traj_advantage_components(
    ...
    metrics_prefix: str = "traj/state_group",
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, float]]:
    """
    Compute Traj advantages with independently weighted episode- and step-skill teacher terms.
    """
    ...
    if step_weight != 0.0:
        if step_rewards is None:
            raise ValueError("step_rewards is required when algorithm.traj.step_advantage_w is non-zero.")
    ...
    step_group_metrics.update({
        "traj/adv/step_advantage_weight": step_weight,
        "traj/adv/episode_skill_teacher_weight": episode_skill_weight,
        "traj/adv/step_skill_teacher_weight": step_skill_weight,
    })
...
def compute_traj_outcome_advantage(token_level_rewards: torch.Tensor,
                                   step_rewards: torch.Tensor,
                                   response_mask: torch.Tensor,
                                   anchor_obs: np.array,
                                   index: np.array,
    ...
    """
    Compute the advantages for Traj.

    Traj uses episode-level outcome advantages plus optional episode-skill and
    step-skill teacher terms, each derived from enhanced-prompt log-probs.
    """
    episode_advantages, step_advantages, teacher_advantages, scores, step_group_metrics = compute_traj_advantage_components(
        token_level_rewards=token_level_rewards,
        ...
        metrics_prefix="traj/state_group",
    )
    if return_metrics:
        component_metrics = summarize_traj_advantage_components(
            response_mask=response_mask,
