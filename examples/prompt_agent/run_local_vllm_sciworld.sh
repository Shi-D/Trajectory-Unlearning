...
DEFAULT_MODEL_PATH="${MODELS_ROOT:+${MODELS_ROOT}/release/traj_3b_sciworld_step80}"
MODEL_PATH="${MODEL_PATH:-$DEFAULT_MODEL_PATH}"
if [[ -z "$MODEL_PATH" ]]; then
  echo "Please set MODEL_PATH, or set MODELS_ROOT so the default Traj model path can be inferred." >&2
  exit 1
fi
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-60001}"
