...
from unlearn.prompting import validate_skill_mode
...
def validate_analysis_prompt_version(version: object) -> str:
    value = str(version or "traj").strip()
    if value not in _ANALYSIS_PROMPT_VERSIONS:
        raise ValueError(
            f"Unsupported Traj analysis_prompt_version: {value!r}. "
...
class TrajEpisodeAnalyzer:
    ...
    def __init__(
        ...
        analysis_prompt_version: str = "traj",
        include_episode_summary: bool = True,
    ):
        self.requested_backend = backend
        if backend == "azure":
            logger.warning("Traj backend='azure' is deprecated; using the OpenAI-compatible backend instead.")
            backend = "openai"
        elif backend not in {"openai", "policy_vllm"}:
            raise ValueError(f"Unsupported Traj backend: {backend}")
        ...
        logger.info(
            "Initialized TrajEpisodeAnalyzer with requested_backend=%s, resolved_backend=%s, model=%s, max_completion_tokens=%s, max_step_skills_per_traj=%s, skill_mode=%s, analysis_prompt_version=%s, include_episode_summary=%s",
    ...
    def _get_openai_client(self):
        if self.backend != "openai":
            raise ValueError(f"Traj backend {self.backend!r} does not use an OpenAI client.")
    ...
    def _build_openai_prompt_with_images(
        ...
        if self.analysis_prompt_version != "traj_visual":
            return prompt
        ...
        if len(images) != placeholder_count:
            raise ValueError(
                f"OpenAI multimodal Traj prompt has {placeholder_count} image placeholder(s) "
    ...
    def _analyze_episode_with_openai(
        ...
        for parse_attempt in range(parse_attempts):
            ...
            try:
                ...
                last_error = exc
                logger.warning(
                    "Traj analyzer JSON parse failed on attempt %s/%s: %s",
                    parse_attempt + 1,
                    parse_attempts,
                    exc,
                )
            ...
            if not self._analysis_has_required_skill(parsed):
                ...
                last_error = ValueError(f"Traj analyzer response missing required field: {missing_field}")
                logger.warning(
                    "Traj analyzer JSON parse failed on attempt %s/%s: %s",
                    parse_attempt + 1,
                    parse_attempts,
        ...
        logger.warning(
            "Traj analyzer failed to produce valid JSON after %s attempt(s); skipping teacher skill for this trajectory: %s",
            parse_attempts,
            last_error,
        )
        return {
    ...
    def _get_parse_attempts(self) -> int:
        raw_attempts = os.environ.get("TRAJ_ANALYSIS_PARSE_RETRIES", "2")
        try:
            return max(1, int(raw_attempts))
        except (TypeError, ValueError):
            logger.warning("Invalid TRAJ_ANALYSIS_PARSE_RETRIES=%r; using 2.", raw_attempts)
            return 2
...
"""
    ...
    def _build_traj_visual_episode_analysis_prompt(
        self,
        steps: List[Dict[str, object]],
        candidate_step_indices: Sequence[int],
        analysis_mode: str = "teacher_bootstrap",
        ...
        for step in steps:
            if not step.get("has_observation_image"):
                raise ValueError("traj_visual analysis requires an observation image for every episode step.")
        image_steps = []
        for step in steps:
    ...
    def _build_episode_analysis_prompt(
        ...
        if self.analysis_prompt_version == "traj_visual":
            return self._build_traj_visual_episode_analysis_prompt(
                steps=steps,
                candidate_step_indices=candidate_step_indices,
                analysis_mode=analysis_mode,
                episode_success=episode_success,
    ...
    def _extract_json_object(self, response: str) -> Dict[str, object]:
        ...
        if not saw_object_start:
            raise ValueError("No JSON object found in Traj analyzer response.")
        raise ValueError(f"Could not parse JSON object in Traj analyzer response: {last_error}") from last_error
