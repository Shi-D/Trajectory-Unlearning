...
def compute_grpo_outcome_advantage(
    ...
    is_injected: Optional[np.ndarray] = None,
    forget_advantage_min: Optional[float] = None,
):
    """
    ...
    Args:
        ...
        is_injected: `(np.ndarray, optional)`
            Traj-unlearn only (see unlearn/unlearn_inject.py). Boolean array,
            shape (bs,), True for rows that are synthetic forget-trajectory
            injections rather than genuine on-policy rollouts. When given,
            these rows are EXCLUDED from the group's own mean/std baseline
            (so an artificially-low injected reward can't inflate the
            advantage of whichever real rollouts happen to share its group
            this step) but are still normalized against that (real-only)
            baseline and assigned their own advantage. None (default)
            reproduces the original behavior exactly -- every other caller
            in this repo leaves this unset.
        forget_advantage_min: `(float, optional)`
            Traj-unlearn only. Floor applied ONLY to rows where is_injected
            is True, after they've been normalized against the real-only
            baseline above -- e.g. -0.1 caps how negative a single injected
            forget row's advantage (and therefore its contribution to the
            policy-gradient loss) can be, regardless of how extreme an
            outlier it is relative to that step's real rollouts. Real rows
            are never affected. Ignored if is_injected is None.
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            if is_injected is not None and is_injected[i]:
                continue
        ...
        for i in range(bsz):
            idx = index[i]
            if idx not in id2mean:
                # Only possible when is_injected excluded every row sharing
                # this group index (e.g. a group consisting solely of
                # injected rows, which shouldn't normally happen since
                # injection always piggybacks onto an existing real
                # group) -- fall back to a neutral baseline rather than
                # crash.
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            ...
            if is_injected is not None and is_injected[i] and forget_advantage_min is not None:
                scores[i] = torch.clamp(scores[i], min=forget_advantage_min)
...
def kl_penalty(logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor, kl_penalty) -> torch.FloatTensor:
    ...
    if kl_penalty in ("low_var_kl", "k3"):
        ...
        # Clamp the exponent, not just kld's output, before exp(): once a
        # token's policy/reference log-prob gap overflows exp() (kl gtr ~88
        # nats), ratio becomes inf. The downstream torch.clamp(kld, -10, 10)
        # saturates the FORWARD value correctly, but its backward pass
        # multiplies the (correctly zeroed) upstream gradient by
        # d(ratio)/d(kl) = exp(kl) = inf, and 0 * inf = nan in IEEE754 --
        # producing a non-finite actor/grad_norm even though every reported
        # loss scalar stays finite. Bounding kl itself keeps exp(kl) (and
        # its own gradient) finite, eliminating that 0*inf path, with no
        # effect on the returned kld value for any non-extreme (unclamped)
        # kl already covered by the existing output-side clamp.
