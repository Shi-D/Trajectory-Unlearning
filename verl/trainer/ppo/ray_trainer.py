...
from unlearn import analysis as core_traj

from unlearn.prompting import (
    SKILL_MODES,
    SKILL_TEACHER_MODES,
...
from unlearn.skill_gen import SkillGenRewardConfig, compute_skill_gen_reward
...
TRAJ_STATE_GROUP_METRIC_PREFIX = "traj/state_group/"
...
def compute_advantage(
    ...
    traj_mode="mean_norm",
    traj_enable_similarity=False,
    traj_similarity_thresh=0.95,
    traj_normalize_teacher_adv=False,
    traj_clip_teacher_adv=None,
    forget_advantage_min=-0.1,
    **kwargs,
):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # Traj-unlearn only: unlearn/unlearn_inject.py's injecting collectors may
        # tag synthetic forget-trajectory rows with "is_forget_injected" --
        # when present, exclude them from each group's own baseline (mean/std)
        # so an injected row's forced-low reward can't inflate the advantage
        # of whichever real rollouts happen to share its group this step, and
        # floor the injected row's own (post-normalization) advantage at
        # forget_advantage_min (caller-supplied, default -0.1 -- see
        # algorithm.traj.forget_advantage_min in ppo_trainer.yaml) so a
        # single outlier group can't drive an unbounded-magnitude policy
        # update from the forget row alone. Absent for every non-unlearn run
        # (.get returns None => unchanged original behavior, floor value
        # ignored when is_injected is None).
        is_injected = data.non_tensor_batch.get("is_forget_injected", None)
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            forget_advantage_min=forget_advantage_min if is_injected is not None else None,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns

        # Traj-unlearn DIAGNOSTIC (temporary, opt-in via
        # TRAJ_PRINT_FORGET_ADVANTAGE=1): print each injected forget row's
        # raw (post baseline-exclusion, pre forget_advantage_min-clamp is
        # already applied inside compute_grpo_outcome_advantage above --
        # this reads the FINAL advantage actually used) advantage, to
        # inspect how extra_penalty (delta) and forget_advantage_min
        # (lambda) interact without needing a full training run.
        if is_injected is not None and os.environ.get("TRAJ_PRINT_FORGET_ADVANTAGE") == "1":
            is_injected_arr = np.asarray(is_injected)
            for i in np.nonzero(is_injected_arr)[0].tolist():
                row_mask = grpo_calculation_mask[i].bool()
                row_adv = advantages[i][row_mask]
                val = row_adv[0].item() if row_adv.numel() > 0 else float("nan")
                print(
                    "Traj-unlearn DIAGNOSTIC: injected row idx=%s raw_advantage=%s "
                    "forget_advantage_min=%s" % (i, val, forget_advantage_min),
                    file=sys.stderr,
                    flush=True,
                )
    ...
    elif adv_estimator == AdvantageEstimator.TRAJ:
        ...
        advantages, returns, traj_adv_metrics = core_gigpo.compute_traj_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            step_rewards=data.batch['step_rewards'],
            response_mask=data.batch['response_mask'],
            anchor_obs=data.non_tensor_batch['anchor_obs'],
            ...
            mode=traj_mode,
            enable_similarity=traj_enable_similarity,
            similarity_thresh=traj_similarity_thresh,
            normalize_teacher_adv=traj_normalize_teacher_adv,
            clip_teacher_adv=traj_clip_teacher_adv,
            return_metrics=True,
        )
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
        if data.meta_info is None:
            data.meta_info = {}
        data.meta_info["traj_adv_metrics"] = traj_adv_metrics
    else:
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._traj_analyzer = None
        self._traj_teacher_adv_last_enabled_state = None
        self._traj_failed_only_last_enabled_state = None
        self._traj_analysis_last_enabled_state = None
        self._traj_teacher_signal_executor = None
    ...
    def _get_traj_opd_stop_after_steps(self) -> Optional[int]:
        stop_after_steps = OmegaConf.select(self.config, "algorithm.traj.opd_stop_after_steps")
        if stop_after_steps is None:
            return None
        return int(stop_after_steps)

    def _get_traj_opd_start_after_steps(self) -> Optional[int]:
        start_after_steps = OmegaConf.select(self.config, "algorithm.traj.opd_start_after_steps")
        if start_after_steps is None:
            # Backward compatibility for older launch commands.
            start_after_steps = OmegaConf.select(self.config, "algorithm.traj.teacher_advantage_start_after_steps")
        if start_after_steps is None:
            return None
        return int(start_after_steps)

    def _get_traj_failed_only_after_steps(self) -> Optional[int]:
        failed_only_after_steps = OmegaConf.select(self.config, "algorithm.traj.failed_only_after_steps")
        if failed_only_after_steps is None:
            return None
        return int(failed_only_after_steps)

    def _should_traj_analyze_failed_only(self) -> bool:
        failed_only_after_steps = self._get_traj_failed_only_after_steps()
        if failed_only_after_steps is None:
            enabled = bool(OmegaConf.select(self.config, "algorithm.traj.failed_only"))
        ...
        if self._traj_failed_only_last_enabled_state != enabled:
            module_logger.info(
                "Traj failed-only episode analysis is %s at global_step=%s (%s).",
                "enabled" if enabled else "disabled",
                self.global_steps,
                schedule_text,
            )
            self._traj_failed_only_last_enabled_state = enabled
        return enabled

    def _is_traj_analysis_enabled(self) -> bool:
        configured = OmegaConf.select(self.config, "algorithm.traj.enable_analysis")
        ...
        if self._traj_analysis_last_enabled_state != enabled:
            module_logger.info(
                "Traj analysis and teacher signal construction are %s at global_step=%s.",
                "enabled" if enabled else "disabled",
                self.global_steps,
            )
            self._traj_analysis_last_enabled_state = enabled
        return enabled

    def _is_traj_policy_vllm_backend(self) -> bool:
        backend = OmegaConf.select(self.config, "algorithm.traj.analysis_backend")
        return str(backend or "") == "policy_vllm"

    def _is_traj_opd_loss_enabled(self) -> bool:
    ...
    def _is_traj_skill_gen_enabled(self) -> bool:
        enabled = OmegaConf.select(self.config, "algorithm.unlearn.skill_gen.enable")
    ...
    def _get_traj_skill_gen_reward_config(self) -> SkillGenRewardConfig:
        def _select(name: str, default):
            value = OmegaConf.select(self.config, f"algorithm.unlearn.skill_gen.{name}")
    ...
    def _collect_traj_skill_gen_samples(
        self,
        episode_analysis: Dict[object, Dict[str, object]],
        analysis_tasks: Dict[object, Dict[str, object]],
        metrics: Dict[str, float],
    ) -> List[Dict[str, object]]:
        if not self._is_traj_skill_gen_enabled():
            metrics["traj/skill_gen_enabled"] = 0.0
            metrics["traj/skill_gen_samples_collected"] = 0.0
            return []

        metrics["traj/skill_gen_enabled"] = 1.0
        if not self._is_traj_policy_vllm_backend():
            metrics["traj/skill_gen_skipped_non_policy_vllm"] = 1.0
            metrics["traj/skill_gen_samples_collected"] = 0.0
            return []
        ...
        max_samples_value = OmegaConf.select(self.config, "algorithm.unlearn.skill_gen.max_samples")
        ...
        metrics["traj/skill_gen_samples_collected"] = float(len(samples))
        metrics["traj/skill_gen_skipped_non_policy_vllm"] = 0.0
        return samples
    ...
    def _stack_skill_gen_tensors(samples: List[Dict[str, object]], key: str, pad_value: int = 0) -> torch.Tensor:
        ...
        if not all(tensor.dim() == 1 for tensor in tensors):
            raise ValueError(f"Cannot pad Traj skill_gen tensor key={key!r} with shapes={sorted(shapes)}.")
    ...
    def _build_traj_skill_gen_payload(
        self,
        batch: DataProto,
        samples: Optional[List[Dict[str, object]]],
    ) -> Optional[Dict[str, object]]:
        if not self._is_traj_skill_gen_enabled() or not samples:
            return None
        ...
        success_threshold = self._get_traj_failure_success_threshold()
        reward_config = self._get_traj_skill_gen_reward_config()
        rewards = []
    ...
    def _is_traj_teacher_signal_enabled(self) -> bool:
        start_after_steps = self._get_traj_opd_start_after_steps()
        stop_after_steps = self._get_traj_opd_stop_after_steps()
        ...
        if self._traj_teacher_adv_last_enabled_state != enabled:
            if enabled:
                schedule_parts = []
                ...
                module_logger.info(
                    "Traj teacher/OPD signal is enabled at global_step=%s%s.",
                    self.global_steps,
                    schedule_text,
                )
            elif disabled_reason == "before_start":
                module_logger.info(
                    "Traj teacher/OPD signal is disabled at global_step=%s until after step %s.",
                    self.global_steps,
                    start_after_steps,
                )
            else:
                module_logger.info(
                    "Traj teacher/OPD signal is disabled at global_step=%s because opd_stop_after_steps=%s.",
                    self.global_steps,
                    stop_after_steps,
                )
            self._traj_teacher_adv_last_enabled_state = enabled
        return enabled

    def _set_zero_traj_teacher_signals(self, batch: DataProto, metrics: Dict[str, float]) -> DataProto:
        ...
        metrics["traj/critical_step_ratio"] = 0.0
        metrics["traj/teacher_batch_size"] = 0.0
        metrics["traj/teacher_available"] = 0.0
        metrics["traj/episode_skill_teacher/enabled"] = 0.0
        metrics["traj/step_skill_teacher/step_skill_step_ratio"] = 0.0
        metrics["traj/step_skill_teacher/step_skills_applied"] = 0.0
        return batch

    def _lazy_init_traj_teacher_signal_executor(self):
        if self._traj_teacher_signal_executor is None:
            self._traj_teacher_signal_executor = ThreadPoolExecutor(
                max_workers=1,
                thread_name_prefix="traj-teacher-signal",
            )
        return self._traj_teacher_signal_executor

    def _build_traj_teacher_signal_snapshot(self, batch: DataProto) -> DataProto:
        tensors = {
    ...
    def _prepare_traj_teacher_signals_async_task(self, batch: DataProto, teacher_enabled: bool):
        local_metrics: Dict[str, float] = {}
        output_batch = self._prepare_traj_teacher_signals(
            batch=batch,
    ...
    def _merge_async_traj_teacher_signals(
        self,
        batch: DataProto,
        ...
        skill_gen_payload = self._build_traj_skill_gen_payload(
            batch=batch,
            samples=teacher_signal_batch.meta_info.get("traj_skill_gen_samples"),
        )
        if skill_gen_payload is not None:
            batch.meta_info["traj_skill_gen"] = skill_gen_payload
        else:
            batch.meta_info.pop("traj_skill_gen", None)
    ...
    def _validate_config(self):
        ...
        opd_stop_after_steps = OmegaConf.select(config, "algorithm.traj.opd_stop_after_steps")
        opd_start_after_steps = OmegaConf.select(config, "algorithm.traj.opd_start_after_steps")
        legacy_teacher_advantage_start_after_steps = OmegaConf.select(
            config,
            "algorithm.traj.teacher_advantage_start_after_steps",
        )
        if opd_start_after_steps is None:
            ...
            module_logger.warning(
                "Both algorithm.traj.opd_start_after_steps=%s and legacy algorithm.traj.teacher_advantage_start_after_steps=%s are set; using opd_start_after_steps.",
                opd_start_after_steps,
                legacy_teacher_advantage_start_after_steps,
            )
        failed_only_after_steps = OmegaConf.select(config, "algorithm.traj.failed_only_after_steps")
        if opd_stop_after_steps is not None and int(opd_stop_after_steps) < 0:
            raise ValueError("algorithm.traj.opd_stop_after_steps must be null or a non-negative integer.")
        if opd_start_after_steps is not None and int(opd_start_after_steps) < 0:
            raise ValueError("algorithm.traj.opd_start_after_steps must be null or a non-negative integer.")
        if failed_only_after_steps is not None and int(failed_only_after_steps) < 0:
            raise ValueError("algorithm.traj.failed_only_after_steps must be null or a non-negative integer.")
        if failed_only_after_steps is not None and bool(OmegaConf.select(config, "algorithm.traj.failed_only")):
            module_logger.warning(
                "algorithm.traj.failed_only_after_steps=%s is set, so scheduled all-then-failed analysis overrides algorithm.traj.failed_only=True until after that step.",
                failed_only_after_steps,
            )
        episode_skill_teacher_advantage_w = float(
            OmegaConf.select(config, "algorithm.traj.episode_skill_teacher_advantage_w") or 0.0
        )
        step_skill_teacher_advantage_w = float(
            OmegaConf.select(config, "algorithm.traj.step_skill_teacher_advantage_w") or 0.0
        )
        skill_mode = str(OmegaConf.select(config, "algorithm.traj.skill_mode") or "episode_step")
        skill_teacher_mode = str(OmegaConf.select(config, "algorithm.traj.skill_teacher_mode") or "step_priority")
        if skill_mode not in SKILL_MODES:
            raise ValueError(
                f"algorithm.traj.skill_mode must be one of {SKILL_MODES}, got {skill_mode!r}."
            )
        if episode_skill_teacher_advantage_w < 0:
            raise ValueError("algorithm.traj.episode_skill_teacher_advantage_w must be non-negative.")
        if step_skill_teacher_advantage_w < 0:
            raise ValueError("algorithm.traj.step_skill_teacher_advantage_w must be non-negative.")
        if skill_teacher_mode not in SKILL_TEACHER_MODES:
            raise ValueError(
                f"algorithm.traj.skill_teacher_mode must be one of {SKILL_TEACHER_MODES}, got {skill_teacher_mode!r}."
            )
        if config.algorithm.adv_estimator == AdvantageEstimator.TRAJ or str(config.algorithm.adv_estimator) == AdvantageEstimator.TRAJ.value:
            analysis_backend = str(OmegaConf.select(config, "algorithm.traj.analysis_backend") or "openai")
            analysis_prompt_version = core_traj.validate_analysis_prompt_version(
                OmegaConf.select(config, "algorithm.traj.analysis_prompt_version") or "traj"
            )
            analysis_enabled_config = OmegaConf.select(config, "algorithm.traj.enable_analysis")
            ...
            if analysis_backend not in {"openai", "policy_vllm"}:
                raise ValueError("algorithm.traj.analysis_backend must be 'openai' or 'policy_vllm'.")
            if analysis_backend == "policy_vllm" and analysis_enabled:
                if str(config.actor_rollout_ref.rollout.name) != "vllm":
                    raise ValueError("algorithm.traj.analysis_backend=policy_vllm requires actor_rollout_ref.rollout.name=vllm.")
                analysis_context_length = int(
                    OmegaConf.select(config, "algorithm.traj.analysis_context_length") or 16384
                )
                analysis_max_completion_tokens = int(
                    OmegaConf.select(config, "algorithm.traj.analysis_max_completion_tokens") or 4096
                )
                ...
                if effective_max_model_len < required_max_model_len:
                    raise ValueError(
                        "policy_vllm Traj analysis requires actor_rollout_ref.rollout.max_model_len "
                        f">= {required_max_model_len}, got {effective_max_model_len}."
                    )
            if float(OmegaConf.select(config, "algorithm.traj.step_advantage_w") or 0.0) != 0.0:
                raise ValueError(
                    "Episode-level Traj OPD requires algorithm.traj.step_advantage_w=0.0."
                )
            if str(OmegaConf.select(config, "algorithm.traj.selector")) != "llm":
                raise ValueError("Episode-level Traj OPD requires algorithm.traj.selector=llm.")
        if opd_start_after_steps is not None and opd_stop_after_steps is not None:
            if int(opd_start_after_steps) >= int(opd_stop_after_steps):
                module_logger.warning(
                    "algorithm.traj.opd_start_after_steps=%s is not earlier than opd_stop_after_steps=%s, so teacher advantage will never be enabled.",
                    opd_start_after_steps,
    ...
    def _get_traj_analysis_dump_dir(self) -> Optional[str]:
        save_analysis = OmegaConf.select(self.config, "algorithm.traj.save_analysis")
        if not save_analysis:
            return None

        dump_dir = OmegaConf.select(self.config, "algorithm.traj.analysis_dump_dir")
        if dump_dir:
            return dump_dir

        return os.path.join(self.config.trainer.default_local_dir, "traj_analysis")

    def _dump_traj_analysis(
        self,
        analysis_tasks: Dict[object, Dict[str, object]],
        episode_analysis: Dict[object, Dict[str, object]],
        selector: str,
    ) -> None:
        dump_dir = self._get_traj_analysis_dump_dir()
        ...
        with open(filename, "w", encoding="utf-8") as f:
            for traj_uid, task in analysis_tasks.items():
                ...
                entry = {
                    ...
                    "skill_mode": analysis.get("skill_mode", self._get_traj_skill_mode()),
                    "analysis_prompt_version": analysis.get(
                        "analysis_prompt_version",
                        self._get_traj_analysis_prompt_version(),
                    ),
                    "analysis_backend_requested": analysis.get(
                        "analysis_backend_requested",
                        self.config.algorithm.traj.analysis_backend,
                    ),
                    ...
                    "include_episode_summary": self._config_bool(
                        self.config,
                        "algorithm.traj.analysis_include_episode_summary",
                        True,
                    ),
        ...
        module_logger.info("Dumped Traj analysis results to %s", filename)

    def _get_traj_augmented_observation_dump_dir(self) -> Optional[str]:
        save_augmented_observations = OmegaConf.select(
            self.config,
            "algorithm.traj.save_augmented_observations",
        )
        ...
        dump_dir = OmegaConf.select(
            self.config,
            "algorithm.traj.augmented_observation_dump_dir",
        )
        if dump_dir:
            return dump_dir
        ...
        return os.path.join(
            ...
            "traj_augmented_observations",
        )

    def _dump_traj_augmented_observations(self, entries: List[Dict[str, object]]) -> None:
        dump_dir = self._get_traj_augmented_observation_dump_dir()
        ...
        module_logger.info("Dumped Traj augmented observations to %s", filename)

    def _get_traj_state_group_dump_dir(self) -> Optional[str]:
        save_state_group_metrics = OmegaConf.select(
            self.config,
            "algorithm.traj.save_state_group_metrics",
        )
        ...
        dump_dir = OmegaConf.select(
            self.config,
            "algorithm.traj.state_group_dump_dir",
        )
        if dump_dir:
            return dump_dir
    ...
    @staticmethod
    def _extract_traj_state_group_histogram(
        state_group_metrics: Dict[str, Any],
    ) -> List[Dict[str, float]]:
        histogram = []
        for key, value in state_group_metrics.items():
            if not key.startswith(TRAJ_STATE_GROUP_METRIC_PREFIX):
                continue
            metric_name = key[len(TRAJ_STATE_GROUP_METRIC_PREFIX):]
            ...
            group_prop = float(
                state_group_metrics.get(
                    f"{TRAJ_STATE_GROUP_METRIC_PREFIX}size_{label}_group_prop",
                    0.0,
                )
            )
            sample_prop = float(
                state_group_metrics.get(
                    f"{TRAJ_STATE_GROUP_METRIC_PREFIX}size_{label}_sample_prop",
                    0.0,
                )
            )
            histogram.append(
    ...
    @staticmethod
    def _build_traj_state_group_svg(
        *,
        global_step: int,
        ...
        title = f"Traj State Group Size Distribution - Step {global_step}"
        subtitle = (
    ...
    def _dump_and_remove_traj_state_group_metrics(self, metrics: Dict[str, Any]) -> None:
        state_group_metrics = {
            key: metrics.pop(key)
            for key in list(metrics.keys())
            if key.startswith(TRAJ_STATE_GROUP_METRIC_PREFIX)
        }
        if not state_group_metrics:
            return

        dump_dir = self._get_traj_state_group_dump_dir()
        if dump_dir is None:
            return
        ...
        histogram = self._extract_traj_state_group_histogram(raw_metrics)
        summary = {
            key[len(TRAJ_STATE_GROUP_METRIC_PREFIX):]: value
            for key, value in raw_metrics.items()
            if key.startswith(TRAJ_STATE_GROUP_METRIC_PREFIX)
            and not key[len(TRAJ_STATE_GROUP_METRIC_PREFIX):].startswith("size_")
            and not key[len(TRAJ_STATE_GROUP_METRIC_PREFIX):].startswith("raw_")
        }
        payload = {
            "global_step": int(self.global_steps),
            "raw_metrics": raw_metrics,
            "raw_group_sizes": raw_metrics.get(
                f"{TRAJ_STATE_GROUP_METRIC_PREFIX}raw_group_sizes",
                [],
            ),
            "histogram": histogram,
            "summary": summary,
        ...
        with open(svg_path, "w", encoding="utf-8") as f:
            f.write(
                self._build_traj_state_group_svg(
                    global_step=int(self.global_steps),
                    histogram=histogram,
                    summary=summary,
                )
        ...
        module_logger.info("Dumped Traj state-group metrics to %s and %s", json_path, svg_path)
    ...
    def _lazy_init_traj_analyzer(self):
        if self._traj_analyzer is None:
            max_step_skills_per_traj = OmegaConf.select(
                self.config,
                "algorithm.traj.analysis_max_step_skills_per_traj",
            )
            ...
            module_logger.info(
                "Initializing Traj analyzer with backend=%s, max_completion_tokens=%s, max_step_skills_per_traj=%s, skill_mode=%s, analysis_prompt_version=%s, include_episode_summary=%s",
                self.config.algorithm.traj.analysis_backend,
                self.config.algorithm.traj.analysis_max_completion_tokens,
                max_step_skills_per_traj,
                self._get_traj_skill_mode(),
                self._get_traj_analysis_prompt_version(),
                include_episode_summary,
            )
            self._traj_analyzer = core_traj.TrajEpisodeAnalyzer(
                backend=self.config.algorithm.traj.analysis_backend,
                max_completion_tokens=self.config.algorithm.traj.analysis_max_completion_tokens,
                max_step_skills_per_traj=max_step_skills_per_traj,
                skill_mode=self._get_traj_skill_mode(),
                analysis_prompt_version=self._get_traj_analysis_prompt_version(),
                include_episode_summary=include_episode_summary,
            )
        return self._traj_analyzer

    def _get_traj_failure_success_threshold(self) -> float:
        threshold = OmegaConf.select(self.config, "algorithm.traj.failure_success_threshold")
        return 1.0 if threshold is None else float(threshold)

    def _get_traj_skill_mode(self) -> str:
        return validate_skill_mode(OmegaConf.select(self.config, "algorithm.traj.skill_mode") or "episode_step")

    def _get_traj_analysis_prompt_version(self) -> str:
        return core_traj.validate_analysis_prompt_version(
            OmegaConf.select(self.config, "algorithm.traj.analysis_prompt_version") or "traj"
        )

    def _build_traj_traj_success_map(self, batch: DataProto) -> Dict[object, float]:
        if "episode_success" in batch.non_tensor_batch:
            ...
            threshold = self._get_traj_failure_success_threshold()
            episode_success_np = (
    ...
    def _traj_prompt_dict_to_text(self, prompt: Dict[str, Any]) -> str:
    ...
    def _extract_policy_vllm_prompt_images(
        ...
        *,
        ...
        if placeholder_count == 0:
            raise RuntimeError("Traj policy_vllm visual analysis prompt has no image placeholders.")
        if placeholder_count != len(images):
            raise RuntimeError(
                "Traj policy_vllm visual analysis prompt has "
    ...
    def _finalize_policy_vllm_traj_analysis(
        self,
        analyzer,
        *,
        content: str,
    ...
    def _analyze_traj_episodes_with_policy_vllm(self, analyzer, analysis_tasks: Dict[object, Dict[str, object]]):
        ...
        for traj_uid in traj_uids:
            ...
            prompt_text = self._traj_prompt_dict_to_text(prompt)
            prompt_texts.append(prompt_text)
            prompt_images.append(
                self._extract_policy_vllm_prompt_images(
                    task["steps"],
                    prompt_text,
                    require_visual_prompt=analyzer.analysis_prompt_version == "traj_visual",
                )
            )

        analysis_context_length = int(
            OmegaConf.select(self.config, "algorithm.traj.analysis_context_length") or 16384
        )
        max_completion_tokens = int(self.config.algorithm.traj.analysis_max_completion_tokens)
        ...
        if use_visual_prompts and not all(bool(images) for images in prompt_images):
            raise RuntimeError(
                "Traj policy_vllm received a mixed visual/text analysis batch."
            )
        ...
        module_logger.info(
            "Traj policy_vllm analysis prompt lengths: min=%s, mean=%.2f, max=%s, max_completion_tokens=%s, visual=%s",
        ...
        for output_idx, traj_uid in enumerate(traj_uids):
            ...
            task = analysis_tasks[traj_uid]
            analysis = self._finalize_policy_vllm_traj_analysis(
                analyzer,
                content=content,
    ...
    def _analyze_traj_episodes(self, analyzer, analysis_tasks: Dict[object, Dict[str, object]]):
        """
        Analyze multiple trajectories for Traj. Azure-backed analysis is run with
        ...
        backend = self.config.algorithm.traj.analysis_backend
        configured_workers = int(self.config.algorithm.traj.analysis_num_workers)
        max_workers = max(1, min(configured_workers, len(analysis_tasks)))

        module_logger.info(
            "Running Traj episode analysis for %s trajectories with backend=%s and num_workers=%s",
            len(analysis_tasks),
            backend,
            max_workers,
        )
        print(f"Traj analysis backend: {backend}, configured_workers: {configured_workers}, max_workers: {max_workers}")

        if backend == "policy_vllm":
            return self._analyze_traj_episodes_with_policy_vllm(
                analyzer=analyzer,
                analysis_tasks=analysis_tasks,
            )
        if backend != "openai":
            raise ValueError(f"Unsupported Traj analysis backend: {backend}")

        results = {}
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="traj-analysis") as executor:
            future_to_traj = {
                executor.submit(
            ...
            for future in as_completed(future_to_traj):
                ...
                try:
                    ...
                    task = analysis_tasks.get(traj_uid, {})
                    module_logger.warning(
                        "Traj episode analysis failed for traj_uid=%s; skipping teacher signal for this trajectory: %s",
                        traj_uid,
                        exc,
                    )
                    results[traj_uid] = {
    ...
    def _prepare_traj_teacher_signals(
        self,
        batch: DataProto,
        metrics: Dict[str, float],
        ...
        """
        Run Traj analysis for the current batch and optionally compute teacher
        ...
        if not self._is_traj_analysis_enabled():
            module_logger.info(
                "Skipping Traj analysis and teacher signal construction for batch_size=%s, num_trajectories=%s.",
                batch_size,
                num_trajectories,
            )
            ...
            metrics["traj/analysis_enabled"] = 0.0
            metrics["traj/analysis_disabled"] = 1.0
            metrics["traj/analysis_num_requests"] = 0.0
            metrics["traj/analysis_num_workers"] = 0.0
            metrics["traj/critical_step_ratio"] = 0.0
            metrics["traj/teacher_batch_size"] = 0.0
            metrics["traj/teacher_available"] = 0.0
            metrics["traj/teacher_skipped_analysis_disabled"] = 1.0
            metrics["traj/episode_skill_teacher/enabled"] = 0.0
            metrics["traj/step_skill_teacher/step_skill_step_ratio"] = 0.0
            metrics["traj/step_skill_teacher/step_skills_applied"] = 0.0
            return batch

        metrics["traj/analysis_enabled"] = 1.0
        metrics["traj/analysis_disabled"] = 0.0
        metrics["traj/episode_skill_teacher/enabled"] = 0.0
        configured_failed_only = bool(OmegaConf.select(self.config, "algorithm.traj.failed_only"))
        failed_only_after_steps = self._get_traj_failed_only_after_steps()
        failed_only = self._should_traj_analyze_failed_only()
        analysis_mode = "failed_episode_opd" if failed_only else "teacher_bootstrap"

        module_logger.info(
            "Preparing Traj analysis for batch_size=%s, num_trajectories=%s, selector=%s, analysis_backend=%s, analysis_mode=%s, failed_only_after_steps=%s",
            batch_size,
            num_trajectories,
            self.config.algorithm.traj.selector,
            self.config.algorithm.traj.analysis_backend,
            analysis_mode,
            failed_only_after_steps,
        )
        metrics["traj/analysis_mode_teacher_bootstrap"] = 1.0 if not failed_only else 0.0
        metrics["traj/analysis_mode_failed_episode_opd"] = 1.0 if failed_only else 0.0

        if "obs_text" not in batch.non_tensor_batch:
            module_logger.warning("Traj teacher signal skipped because obs_text is missing from the rollout batch.")
            batch.batch["teacher_log_prob"] = zero_teacher_log_prob
            batch.batch["episode_teacher_log_prob"] = zero_teacher_log_prob.clone()
            batch.batch["step_teacher_log_prob"] = zero_teacher_log_prob.clone()
            batch.batch["critical_step_mask"] = zero_critical_mask
        ...
        step_indices = core_traj.build_traj_step_indices(batch.non_tensor_batch["traj_uid"])
        batch.non_tensor_batch["step_idx"] = step_indices

        selector = self.config.algorithm.traj.selector
        analyzer = self._lazy_init_traj_analyzer()
        metrics["traj/failed_only"] = 1.0 if failed_only else 0.0
        metrics["traj/failed_only_config"] = 1.0 if configured_failed_only else 0.0
        metrics["traj/failed_only_after_steps"] = (
            float(failed_only_after_steps) if failed_only_after_steps is not None else -1.0
        )
        metrics["traj/failed_only_schedule_active"] = 1.0 if failed_only_after_steps is not None else 0.0
        metrics["traj/failure_success_threshold"] = self._get_traj_failure_success_threshold()
        ...
        if analyzer.analysis_prompt_version == "traj_visual":
            if analysis_obs_images is None or any(image is None for image in analysis_obs_images):
                raise RuntimeError(
                    "traj_visual Traj analysis requires an observation image for every rollout step."
                )

        episodes = core_traj.build_episode_records(
        ...
        if episodes:
            ...
            module_logger.info(
                "Built Traj episode records for %s trajectories (min_steps=%s, mean_steps=%.2f, max_steps=%s)",
                len(episodes),
                min(episode_lengths),
            ...
            module_logger.info("No Traj episode records were built for the current batch.")

        episode_analysis: Dict[object, Dict[str, object]] = {}
        analysis_tasks: Dict[object, Dict[str, object]] = {}
        traj_success = self._build_traj_traj_success_map(batch)
        if failed_only and not traj_success:
            module_logger.warning(
                "Traj failed_only is enabled, but episode_success/episode_rewards are missing; analyzing all trajectories."
            )
        ...
        metrics["traj/analyzed_traj_count"] = analyzed_traj_count
        metrics["traj/failed_traj_count"] = analyzed_traj_count
        metrics["traj/skipped_success_traj_count"] = float(
        ...
        module_logger.info(
            "Traj LLM analyzer will build episode-level teacher skills for %s/%s trajectories.",
        ...
        analyzed, analysis_workers = self._analyze_traj_episodes(analyzer=analyzer, analysis_tasks=analysis_tasks)
        episode_analysis.update(analyzed)
        skill_gen_samples = self._collect_traj_skill_gen_samples(
            episode_analysis=episode_analysis,
            analysis_tasks=analysis_tasks,
            metrics=metrics,
        )
        if skill_gen_samples:
            batch.meta_info["traj_skill_gen_samples"] = skill_gen_samples
        else:
            batch.meta_info.pop("traj_skill_gen_samples", None)

        def _has_successful_analysis(traj_uid: object, analysis: Dict[str, object]) -> bool:
            if analysis.get("analysis_error"):
                return False
            if self._get_traj_skill_mode() == "step_only":
        ...
        module_logger.info(
            "Traj episode-level OPD selected %s steps from %s successful analyzed trajectories (%s failed/skipped).",
            int(critical_mask_np.sum()),
            len(analyzed_traj_uids),
            failed_analysis_count,
        )
        metrics["traj/analysis_num_requests"] = float(len(analysis_tasks))
        metrics["traj/analysis_num_workers"] = float(analysis_workers)
        metrics["traj/analysis_succeeded_traj_count"] = float(len(successful_episode_analysis))
        metrics["traj/analysis_failed_traj_count"] = float(failed_analysis_count)
        module_logger.info(
            "Traj episode analysis finished: requests=%s, workers=%s, successful_trajectories=%s, failed_trajectories=%s",
            len(analysis_tasks),
            analysis_workers,
            len(successful_episode_analysis),
            failed_analysis_count,
        )
        self._dump_traj_analysis(
            analysis_tasks=analysis_tasks,
            episode_analysis=episode_analysis,
            selector=selector,
        )
        ...
        metrics["traj/critical_step_ratio"] = float(critical_mask_np.mean()) if batch_size > 0 else 0.0
        metrics["traj/teacher_batch_size"] = float(len(critical_indices))
        metrics["traj/teacher_available"] = 1.0
        metrics["traj/step_skill_teacher/step_skill_step_ratio"] = 0.0
        metrics["traj/step_skill_teacher/step_skills_applied"] = 0.0
        module_logger.info(
            "Traj finalized %s OPD-scored steps (step_ratio=%.4f, analysis_mode=%s).",
            len(critical_indices),
            metrics["traj/critical_step_ratio"],
            analysis_mode,
        )
        ...
        if not teacher_enabled:
            ...
            metrics["traj/teacher_batch_size"] = 0.0
            metrics["traj/teacher_available"] = 0.0
            teacher_start_after_steps = self._get_traj_opd_start_after_steps()
            teacher_stop_after_steps = self._get_traj_opd_stop_after_steps()
            metrics["traj/teacher_skipped_by_schedule"] = 1.0
            metrics["traj/teacher_skipped_before_start"] = (
                1.0
                if teacher_start_after_steps is not None and self.global_steps <= teacher_start_after_steps
                else 0.0
            )
            metrics["traj/teacher_skipped_after_opd_stop"] = (
                1.0
        ...
        if len(critical_indices) == 0:
            module_logger.info("Traj has no episode-level OPD steps for the current batch.")
            batch.batch["teacher_log_prob"] = zero_teacher_log_prob
            batch.batch["episode_teacher_log_prob"] = zero_teacher_log_prob.clone()
            batch.batch["step_teacher_log_prob"] = zero_teacher_log_prob.clone()
            batch.batch["step_skill_mask"] = zero_step_skill_mask
        ...
        metrics["traj/teacher_multimodal"] = 1.0 if teacher_obs_images is not None else 0.0
        opd_loss_enabled = self._is_traj_opd_loss_enabled()
        skill_mode = self._get_traj_skill_mode()
        episode_skill_teacher_weight = float(
            OmegaConf.select(self.config, "algorithm.traj.episode_skill_teacher_advantage_w") or 0.0
        )
        ...
        step_skill_teacher_enabled = (
            float(OmegaConf.select(self.config, "algorithm.traj.step_skill_teacher_advantage_w") or 0.0) > 0.0
            or opd_loss_enabled
        )
        if skill_mode == "episode_only":
            step_skill_teacher_enabled = False
        skill_teacher_mode = str(
            OmegaConf.select(self.config, "algorithm.traj.skill_teacher_mode") or "step_priority"
        )
        metrics["traj/episode_skill_teacher/enabled"] = 1.0 if episode_skill_teacher_enabled else 0.0
        metrics["traj/opd_loss_enabled"] = 1.0 if opd_loss_enabled else 0.0
        metrics["traj/episode_skill_teacher_skipped_zero_weight"] = (
            0.0 if episode_skill_teacher_enabled else 1.0
        )
        metrics["traj/skill_mode_step_only"] = 1.0 if skill_mode == "step_only" else 0.0
        metrics["traj/skill_mode_episode_only"] = 1.0 if skill_mode == "episode_only" else 0.0
        metrics["traj/skill_teacher_mode_additive"] = 1.0 if skill_teacher_mode == "additive" else 0.0
        ...
        if len(critical_indices) > 0:
            metrics["traj/step_skill_teacher/step_skill_step_ratio"] = float(
                step_skill_guided_steps / len(critical_indices)
            )
            metrics["traj/episode_skill_teacher/episode_skill_step_ratio"] = float(
                len(episode_skill_indices) / len(critical_indices)
            )
        metrics["traj/step_skill_teacher/step_skills_applied"] = float(step_skill_guided_steps)
        metrics["traj/episode_skill_teacher/episode_skills_applied"] = float(len(episode_skill_indices))
        metrics["traj/teacher_batch_size"] = float(len(episode_skill_indices) + len(step_skill_indices))
        metrics["traj/teacher_available"] = 1.0 if (episode_skill_indices or step_skill_indices) else 0.0

        module_logger.info(
            "Traj built %s episode-skill and %s step-skill observations for teacher scoring across %s trajectories.",
        ...
        if critical_preview and module_logger.isEnabledFor(logging.DEBUG):
            module_logger.debug("Traj episode-level OPD preview: %s", critical_preview)
        self._dump_traj_augmented_observations(augmented_observation_dump_entries)
        visual_teacher_required = analyzer.analysis_prompt_version == "traj_visual"
        ...
        def _compute_skill_log_probs(
            *,
            ...
            teacher_meta_info.pop("traj_skill_gen_samples", None)
            ...
            if visual_teacher_required and (
                ...
                raise RuntimeError("traj_visual Traj teacher scoring requires an image for every prompt.")
            if use_prompt_images and not all(image is not None for image in prompt_images):
                raise RuntimeError(
                    "Traj %s teacher scoring received a mixed visual/text prompt batch."
                    % label
                )
            ...
            module_logger.info(
                "Traj %s teacher prompt lengths: min=%s, mean=%.2f, max=%s, visual=%s",
                label,
                int(prompt_lengths.min()),
                float(prompt_lengths.mean()),
                int(prompt_lengths.max()),
        ...
        metrics["traj/teacher_log_prob_mean"] = 0.0
        metrics["traj/episode_skill_teacher_log_prob_mean"] = 0.0
        if episode_skill_indices:
            ...
            metrics["traj/episode_skill_teacher_log_prob_mean"] = float(
        ...
        metrics["traj/step_skill_teacher_log_prob_mean"] = 0.0
        if step_skill_indices:
            ...
            metrics["traj/step_skill_teacher_log_prob_mean"] = float(
        ...
        if active_teacher_log_prob_chunks:
            metrics["traj/teacher_log_prob_mean"] = float(
        ...
        if episode_skill_indices:
            module_logger.info(
                "Traj computed episode-skill teacher log-probs for %s steps (token_mean=%.6f, token_min=%.6f, token_max=%.6f).",
            ...
            module_logger.info(
                "Traj skipped episode-skill teacher log-probs because episode_skill_teacher_advantage_w is %.6f.",
                episode_skill_teacher_weight,
            )
        else:
            module_logger.info("Traj skipped episode-skill teacher log-probs because all OPD-scored steps have step skills.")
        return batch
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    traj_teacher_future = None
                    traj_teacher_snapshot = None
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ:
                        traj_teacher_schedule_enabled = self._is_traj_teacher_signal_enabled()
                        traj_analysis_enabled = self._is_traj_analysis_enabled()
                        traj_opd_loss_enabled = self._is_traj_opd_loss_enabled()
                        traj_skill_gen_enabled = self._is_traj_skill_gen_enabled()
                        traj_teacher_signal_enabled = traj_teacher_schedule_enabled and traj_analysis_enabled
                        traj_teacher_adv_enabled = traj_teacher_signal_enabled and not traj_opd_loss_enabled
                        traj_policy_vllm_backend = self._is_traj_policy_vllm_backend()
                    # generate a batch
                    ...
                    if self.config.algorithm.adv_estimator in [AdvantageEstimator.GiGPO, AdvantageEstimator.TRAJ]:
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ:
                        metrics["traj/teacher_enabled"] = 1.0 if traj_teacher_signal_enabled else 0.0
                        metrics["traj/teacher_signal_enabled"] = 1.0 if traj_teacher_signal_enabled else 0.0
                        metrics["traj/teacher_advantage_enabled"] = 1.0 if traj_teacher_adv_enabled else 0.0
                        metrics["traj/opd_loss_enabled"] = 1.0 if traj_opd_loss_enabled else 0.0
                        metrics["traj/skill_gen_enabled"] = 1.0 if traj_skill_gen_enabled else 0.0
                        metrics["traj/teacher_disabled_by_schedule"] = 0.0 if traj_teacher_schedule_enabled else 1.0
                        metrics["traj/teacher_disabled_by_analysis"] = (
                            1.0
                            if traj_teacher_schedule_enabled and not traj_analysis_enabled
                            else 0.0
                        )
                        metrics["traj/analysis_enabled"] = 1.0 if traj_analysis_enabled else 0.0
                        metrics["traj/analysis_disabled"] = 0.0 if traj_analysis_enabled else 1.0
                        if traj_analysis_enabled:
                            traj_teacher_snapshot = self._build_traj_teacher_signal_snapshot(batch)
                            if not traj_policy_vllm_backend:
                                traj_teacher_future = self._lazy_init_traj_teacher_signal_executor().submit(
                                    self._prepare_traj_teacher_signals_async_task,
                                    traj_teacher_snapshot,
                                    traj_teacher_signal_enabled,
                                )
                                traj_teacher_snapshot = None
                    ...
                    batch = adjust_batch(
                        self.config,
                        batch,
                        track_source_indices=self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ,
                    )
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ:
                        with _timer("traj_teacher", timing_raw):
                            if traj_teacher_future is None and traj_teacher_snapshot is None:
                                module_logger.info(
                                    "Traj analysis is disabled; using zero teacher signals for this batch."
                                )
                                batch = self._set_zero_traj_teacher_signals(batch=batch, metrics=metrics)
                                metrics["traj/analysis_enabled"] = 0.0
                                metrics["traj/analysis_disabled"] = 1.0
                                metrics["traj/analysis_num_requests"] = 0.0
                                metrics["traj/analysis_num_workers"] = 0.0
                                metrics["traj/teacher_skipped_analysis_disabled"] = 1.0
                                batch.non_tensor_batch.pop("_batch_source_idx", None)
                            elif traj_teacher_snapshot is not None:
                                teacher_signal_batch, teacher_signal_metrics = self._prepare_traj_teacher_signals_async_task(
                                    traj_teacher_snapshot,
                                    traj_teacher_signal_enabled,
                                )
                                metrics.update(teacher_signal_metrics)
                                batch = self._merge_async_traj_teacher_signals(
                                    batch=batch,
                                    teacher_signal_batch=teacher_signal_batch,
                                )
                            else:
                                try:
                                    teacher_signal_batch, teacher_signal_metrics = traj_teacher_future.result()
                                    metrics.update(teacher_signal_metrics)
                                    batch = self._merge_async_traj_teacher_signals(
                                        batch=batch,
                                        teacher_signal_batch=teacher_signal_batch,
                                    ...
                                    if (
                                        str(OmegaConf.select(self.config, "algorithm.traj.analysis_prompt_version") or "")
                                        == "traj_visual"
                                    ):
                                        raise
                                    module_logger.warning(
                                        "Asynchronous Traj teacher signal preparation failed; falling back to zero teacher signals for this batch: %s",
                                        exc,
                                    )
                                    batch.non_tensor_batch.pop("_batch_source_idx", None)
                                    batch = self._set_zero_traj_teacher_signals(batch=batch, metrics=metrics)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ:
                            episode_skill_teacher_advantage_w = (
                                float(OmegaConf.select(self.config, "algorithm.traj.episode_skill_teacher_advantage_w") or 0.0)
                                if traj_teacher_adv_enabled
                                else 0.0
                            )
                            step_skill_teacher_advantage_w = (
                                float(OmegaConf.select(self.config, "algorithm.traj.step_skill_teacher_advantage_w") or 0.0)
                                if traj_teacher_adv_enabled
                                else 0.0
                            )
                            metrics["traj/episode_skill_teacher_advantage_w_current"] = float(episode_skill_teacher_advantage_w)
                            metrics["traj/step_skill_teacher_advantage_w_current"] = float(step_skill_teacher_advantage_w)
                        else:
                        ...
                        batch = compute_advantage(
                            ...
                            step_advantage_w=(
                                self.config.algorithm.traj.step_advantage_w
                                if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ
                            ...
                            episode_skill_teacher_advantage_w=episode_skill_teacher_advantage_w,
                            step_skill_teacher_advantage_w=step_skill_teacher_advantage_w,
                            traj_mode=self.config.algorithm.traj.mode,
                            traj_enable_similarity=self.config.algorithm.traj.enable_similarity,
                            traj_similarity_thresh=self.config.algorithm.traj.similarity_thresh,
                            traj_normalize_teacher_adv=self.config.algorithm.traj.normalize_teacher_adv,
                            traj_clip_teacher_adv=self.config.algorithm.traj.clip_teacher_adv,
                            # None/null (Hydra: algorithm.traj.forget_advantage_min=null)
                            # disables the floor entirely -- compute_grpo_outcome_advantage
                            # only clamps when this is not None.
                            forget_advantage_min=(
                                float(self.config.algorithm.traj.forget_advantage_min)
                                if self.config.algorithm.traj.forget_advantage_min is not None
                                else None
                            ),
                        )
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.TRAJ:
                            metrics.update(batch.meta_info.pop("traj_adv_metrics", {}))
                        elif self.config.algorithm.adv_estimator == AdvantageEstimator.GiGPO:
                            metrics.update(batch.meta_info.pop("gigpo_adv_metrics", {}))

                        # Traj-unlearn DIAGNOSTIC (temporary, opt-in via
                        # TRAJ_PRINT_FORGET_ADVANTAGE=1): print each injected
                        # forget row's raw final advantage as actually used
                        # for this step's policy update, regardless of
                        # which adv_estimator branch computed it above (the
                        # GRPO-only forget_advantage_min clamp inside
                        # compute_advantage does NOT apply to the Traj/GiGPO
                        # branches -- this reads whatever ended up in
                        # batch.batch["advantages"] instead of assuming any
                        # particular estimator's internals).
                        if os.environ.get("TRAJ_PRINT_FORGET_ADVANTAGE") == "1":
                            is_injected_diag = batch.non_tensor_batch.get("is_forget_injected", None)
                            if is_injected_diag is not None:
                                is_injected_arr = np.asarray(is_injected_diag)
                                grpo_calc_mask_diag = batch.batch["response_mask"]
                                for i in np.nonzero(is_injected_arr)[0].tolist():
                                    row_mask = grpo_calc_mask_diag[i].bool()
                                    row_adv = batch.batch["advantages"][i][row_mask]
                                    val = row_adv[0].item() if row_adv.numel() > 0 else float("nan")
                                    print(
                                        "Traj-unlearn DIAGNOSTIC: injected row idx=%s raw_advantage=%s "
                                        "adv_estimator=%s forget_advantage_min=%s"
                                        % (
                                            i,
                                            val,
                                            self.config.algorithm.adv_estimator,
                                            self.config.algorithm.traj.forget_advantage_min,
                                        ),
                                        file=sys.stderr,
                                        flush=True,
                                    )
                ...
                self._dump_and_remove_traj_state_group_metrics(metrics)
